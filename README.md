...
11. Click **Categorize WAV** *(optional)* to sort WAV files into `bgm`, `cs`, `se`, character-name, or `etc` folders. Scenario WAVs go into `scenario/<character>` or `scenario/etc`; files already at the top of `scenario` are also classified. Matching is case-insensitive; precedence is `bgm_`, `CS_`, `se_`, `s<digits>_`, character name, then `etc`.
